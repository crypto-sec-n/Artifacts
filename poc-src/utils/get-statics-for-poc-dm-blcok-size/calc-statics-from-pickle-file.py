...
ax1.bar(final_values, final_frequencies, color='b')
ax1.set_xlabel('Ciphertext size (Byte)')
ax1.set_ylabel('Posts (DMs)', color='b')

# 二つ目のy軸を追加（0.0 - 1.0の割合）
ax2 = ax1.twinx()
total_count = sum(final_frequencies)
ratios = [(f / total_count)*100.0 for f in final_frequencies]

ax2.bar(final_values, ratios, color='b', alpha=0.0)
ax2.set_ylabel('Percentage (%)', color='b')
#ax2.plot(final_values, ratios, color='g')
